...
public static class ImprimanteBrute
{
    ...
    private static Resultat EnvoyerFichier(byte[] flux, string raison)
    {
        ...
        {
            return new Resultat(false, $"erreur:{chemin}",
                $"imprimante indisponible et sauvegarde locale impossible : {ex.Message}");
        }
    }
    ...
    private static void EnvoyerWinspool(byte[] flux, string nomImprimante)
    {
        ...
        try
        {
            ...
            {
                pDocName = $"POSTEC ticket {DateTime.Now:yyyyMMdd HHmmss}",
                pOutputFile = null!,
                pDatatype = "RAW",
            };
