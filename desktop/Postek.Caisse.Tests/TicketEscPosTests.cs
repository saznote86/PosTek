...
public class TicketEscPosTests
{
    ...
    [Fact]
    public void Ticket_ContientIdentiteFiscaleTimbreEtCodeBarres()
    {
        var donnees = new DonneesTicket
        {
            ...
            Adresse = "Tunis",
            InfosSociete = new[] { "POSTEC SARL" },
            MatriculeFiscal = "1234567/A/M/000",
            SousTotalHt = 2.017m,
            TimbreFiscal = 1.000m,
            CodeBarres = "0000000012",
        };

        var flux = TicketEscPos.Generer(donnees);
        var texte = Decode(flux).Texte;

        Assert.Contains("POSTEC SARL", texte);
        Assert.Contains("MF : 1234567/A/M/000", texte);
        Assert.Contains("TIMBRE FISCAL", texte);
        Assert.Contains("SOUS-TOTAL HT", texte);
        Assert.Contains((byte)0x1D, flux); // commandes Code 128 ESC/POS
        Assert.Contains((byte)73, flux);   // GS k 73 = Code 128
    }
    ...
    public void Accents_EncodesCP863_SansOctetInvalide()
    {
        ...
        // 'é' → 0x82 (CP-850/863 imprimante), mappage manuel POSTEC.
